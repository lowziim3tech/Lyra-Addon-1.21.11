...
public final class SeaTechScreen extends Screen {
    private ButtonWidget chop;
    private ButtonWidget camera;
    public SeaTechScreen() { super(Text.literal("SeaTech Auto Sell")); }
    @Override protected void init() {
        int x = width / 2 - 100;
        int y = height / 2 - 40;
        chop = addDrawableChild(ButtonWidget.builder(Text.empty(), b -> {
            SeaTechClient.INSTANCE.toggle(); refresh();
        }).dimensions(x, y, 200, 20).build());
        camera = addDrawableChild(ButtonWidget.builder(Text.empty(), b -> {
            SeaTechClient.INSTANCE.toggleCamera(); refresh();
    ...
    private void refresh() {
        chop.setMessage(Text.literal("Auto Chop: " + (SeaTechClient.INSTANCE.enabled() ? "ON" : "OFF")));
        camera.setMessage(Text.literal("Look Up / 2 min: " + (SeaTechClient.INSTANCE.correctCamera() ? "ON" : "OFF")));
    }
    @Override public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        super.render(context, mouseX, mouseY, delta);
        context.drawCenteredTextWithShadow(textRenderer, title, width / 2, height / 2 - 70, 0xFFFFFFFF);
        context.drawCenteredTextWithShadow(textRenderer, Text.literal(SeaTechClient.INSTANCE.status()), width / 2, height / 2 + 15, 0xFFAAAAAA);
